...
class ProductSearchExportStructureFacadeTest extends TestCase
{
    ...
    {
        $this->client = $this->createMock(MicroserviceClient::class);
        $this->domain = $this->createMock(Domain::class);
        $this->facade = new ProductSearchExportStructureFacade($this->client, $this->domain);
    }

    public function testCreateSuccessIndex(): void
    {
        $this->domain->method('getAllIds')->willReturn([1, 2]);
        $this->client->method('post')->withConsecutive([1], [2]);
    ...
    {
        $this->domain->method('getAllIds')->willReturn([1, 2]);
        $this->client->method('delete')->withConsecutive([1], [2]);
